							...
							<div class="col-md-12"
								...
								<div class="team d-md-flex p-4 bg-white">
									...
									<div class="text pl-md-4">
										<a
											href="<c:url value='/detail-post/' />${user.id}">
											<h5 class="location mb-0">${user.fullName}</h5>
											<p style="display: block; color: black">${user.address}</p> <span
											class="position" style="display: block; color: black">${user.email}</span>
							...
							<div class="block-27">
								<ul>
									<c:if test="${numberPage > 0}">
										<li><a
											href="/user/list-candidate/?page=${list.number - 1}">&lt;</a></li>
									</c:if>
									<c:forEach var="state" items="${recruitmentList}"
										varStatus="status">
										<li class="${numberPage == status.index ? 'active' : ''}">
											<a href="/user/list-candidate/?page=${status.index}">${status.index + 1}</a>
										</li>
									</c:forEach>
