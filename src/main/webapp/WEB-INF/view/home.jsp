...
<body>
				...
				<ul class="navbar-nav ml-auto">
					...
					<div class="dropdown">
						<button class="btn btn-secondary dropdown-toggle" type="button"
							...
							aria-haspopup="true" aria-expanded="false">${user.fullName}</button>
						<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
							<a class="dropdown-item" href="<c:url value="/profile" />">Hồ
								Sơ</a>
							<security:authorize access="hasRole('MANAGER')">
								<a class="dropdown-item" href="<c:url value="/post-list" />">Danh
									sách bài đăng</a>
							</security:authorize>
	...
	<!-- END nav -->
	<div if="${msg_register_success}" class="toast" data-delay="2000"
								...
								<div class="col-md-12 nav-link-wrap">
									<div class="nav nav-pills text-center" id="v-pills-tab"
										...
										<a class="nav-link active mr-md-1" id="v-pills-1-tab"
											...
											việc</a> <a class="nav-link" id="v-pills-1-tab"
											data-toggle="pill" href="#v-pills-1" role="tab"
											aria-controls="v-pills-1" aria-selected="false">Tìm theo
								...
								<div class="col-md-12 tab-wrap">
									...
									<div class="tab-content p-4" id="v-pills-tabContent">
										...
										<div class="tab-pane fade show active" id="v-pills-1"
											...
											<form action="<c:url value='/result-search'/>" method="get"
												class="search-job">
										...
										<div class="tab-pane fade" id="v-pills-3" role="tabpanel"
											aria-labelledby="v-pills-performance-tab">
											<form action="<c:url value='/result-search-address'/>"
												method="get" class="search-job">
												<div class="row no-gutters">
					...
					<div class="col-md-3">
						<ul class="category text-center">
							<li><a style="text-decoration: none !important;"
								href="${'/recruitment/category/'}${categorie.id}">
									<p>
										<c:out value="${categorie.name}" />
									</p> <span class="number"><c:out
											value="${categorie.numberChoose}" /></span> <span>Vị trí</span> <i
				...
				<div class="col-lg-9 pr-lg-5">
					...
					<div class="row">
						<c:forEach var="recruitment" items="${recruitments}">
							<div class="col-md-12 ">
								<input type="hidden" class="form-control" name="recruitmentId"
								...
								<div
									...
									<div class="one-third mb-4 mb-md-0">
										<div class="job-post-item-header align-items-center">
											...
											<h2 class="mr-3 text-black">
												<a href="<c:url value='/detail-post/' />${recruitment.id}">
										...
										<div class="job-post-item-body d-block d-md-flex">
											<div class="mr-3">
												<span class="icon-layers"></span> <a href="#"> <c:out
														value="${recruitment.company.nameCompany}" />
												</a>
											</div>
											<div>
									...
									<input type="hidden"
										id="idRe<c:out value="${recruitment.id}" />"
										value="<c:out value="${recruitment.id}" />">


									<div
										class="one-forth ml-auto d-flex align-items-center mt-4 md-md-0">
										<c:choose>
											<c:when test="${recruitment.status == 1}">
												<div>
													<a
														href="${pageContext.request.contextPath}/saveStatus/${recruitment.id}"
														id="icon${recruitment.id}"
														onclick="save(${recruitment.id})"
										...
										<c:forEach var="applypost" items="${applyposts}">
											<c:if test="${applypost.recruitment.id eq recruitment.id}">
										...
										<c:choose>
											<c:when test="${isApplied eq true}">
												<div class="applied-job-message">Đã apply</div>
											</c:when>
											<c:otherwise>
							...
							<div class="modal fade"
								id="exampleModal<c:out value="${recruitment.id}" />"
									...
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="exampleModalLabel">
												Ứng tuyển: <span><c:out value="${recruitment.title}" /></span>
											</h5>
										...
										<form method="post"
											...
											<input type="hidden" name="${_csrf.parameterName}"
												...
												type="hidden" class="form-control" name="recruitmentId"
												value="${recruitment.id}" />
												...
												<div class="row">
													...
													<div id="loai1${recruitment.id}" style="display: none"
														...
														<textarea rows="10" cols="3" class="form-control"
															id="text${recruitment.id}" name="introduction"></textarea>
													</div>
													<div id="loai2${recruitment.id}" style="display: none"
														...
														<label for="fileUpload" class="col-form-label">Chọn
															...
															id="fileUpload${recruitment.id}" name="file"> <label
											...
											<div class="modal-footer">
												...
												<button type="submit" id="button1${recruitment.id}"
													style="display: none" class="btn btn-primary">Nộp</button>
												<button type="submit" id="button2${recruitment.id}"
													style="display: none" class="btn btn-primary">Nộp</button>
											</div>
										</form>
				...
				<div class="col-lg-3 sidebar">
					...
					<c:forEach var="company" items="${company}">
						<div class="sidebar-box">
							<div>
								<a
									href="${pageContext.request.contextPath}/post-company/${company.id}"
									class="company-wrap"> <img
									src="${pageContext.request.contextPath}/resources/images/${company.logo}"
									class="img-fluid" alt="Colorlib Free Template">
								</a>
								<div class="text p-3">
									<h3>
										<a
											href="${pageContext.request.contextPath}/post-company/${company.id}">${company.nameCompany}
									</h3>
									<p>
										<span class="number" style="color: black"> <c:out
												value="${company.recruitments.get(0).quantity}" />
										</span> <span>Vị trí ứng tuyển</span>
									</p>
								</div>
							</div>
						</div>
	...
	<script>
	  ...
	  function save(recruitmentId) {
		    var iconElement = document.getElementById("icon" + recruitmentId);
		    var isSaved = iconElement.classList.contains("saved");

		    if (isSaved) {
		      // Remove saved class
		      iconElement.classList.remove("saved");
		      // Display success message for removing from saved list
		      alert("Bỏ lưu theo dõi thành công.");
		    } else {
		      // Add saved class
		      iconElement.classList.add("saved");
		      // Display success message for saving to the list
		      alert("Lưu theo dõi thành công.");
		    }
		  } 
    function choosed(id){
        var name = '#choose' + id;
